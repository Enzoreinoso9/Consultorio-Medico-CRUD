...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelector("table.paciente").addEventListener("click", function (event) {
        if (event.target.classList.contains("editarBtn")) {
            event.preventDefault();
            var row = event.target.closest("tr");
            var idPaciente = row.cells[0].innerText;
            ...
            var idMedico = row.cells[7].innerText;
            var informe = row.cells[8].innerText;
            ...
            document.getElementById('informeEditar').value = informe;
